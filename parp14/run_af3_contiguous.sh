#!/bin/bash
#
# Run AF3 for 18 missing CONTIGUOUS domain constructs.
# These are all N-terminal extensions (RRM1/2/3 + core) that fill out
# the complete set of contiguous domain subsequences.
#
# Two-phase: MSA on CPU (parallel), inference on GPU (model loads once).
# Handles timestamped AF3 output dirs. Skips completed MSAs and models.
# Uses existing JSONs from alphafold_inputs_missing/ (no regeneration).
#
# Usage:
#   ./run_af3_contiguous.sh                      # 2 GPUs, 8 MSA parallel
#   ./run_af3_contiguous.sh --gpu0 0 --gpu1 1    # specify GPU IDs
#   ./run_af3_contiguous.sh --msa-parallel 12    # control CPU parallelism
#   ./run_af3_contiguous.sh --skip-msa           # skip MSA (already done)
#   ./run_af3_contiguous.sh --msa-only           # run MSA only, no inference
#   ./run_af3_contiguous.sh --dry-run            # show plan without running
#
# Monitor:
#   tail -f alphafold_outputs/logs/msa_*.log
#   tail -f run_af3_gpu0_contiguous.log
#   tail -f run_af3_gpu1_contiguous.log
...
# Paths
LIST_FILE="$SCRIPT_DIR/missing_contiguous.txt"
...
echo "============================================================================"
echo "AF3 Contiguous Domain Constructs (18 missing)"
echo "============================================================================"
...
if [ $SKIP_INFERENCE -eq 0 ] && [ $N_INF_TOTAL -gt 0 ]; then
    ...
    run_gpu_inference() {
        ...
        local label=$2
        local batch_dir=$3
        local log_file="$SCRIPT_DIR/run_af3_gpu${label}_contiguous.log"
    ...
    echo "  Monitor: tail -f $SCRIPT_DIR/run_af3_gpu0_contiguous.log"
    echo "           tail -f $SCRIPT_DIR/run_af3_gpu1_contiguous.log"
    echo ""
...
if [ "${FAILURES:-0}" -gt 0 ]; then
    echo "  WARNING: $FAILURES GPU(s) had errors. Check contiguous logs."
fi
echo "============================================================================"
