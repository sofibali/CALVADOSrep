...
def _flat_replicate_dirs(folder):
    """Every immediate subdirectory of `folder` containing any .dcd file.
...
def _resolve_sim_dir(set_key, seed, sample):
    if set_key in EXTERNAL_DIRS:
        ...
        if not sim_dir.is_dir():
            ...
            if idx < len(flat):
                sim_dir = flat[idx]
        return sim_dir
    return CWD / set_key / f'seed-{seed}_sample-{sample}'


def _resolve_topology_pdb(sim_dir):
...
def worker_contacts(set_key, seed, sample):
    ...
    sim_dir = _resolve_sim_dir(set_key, seed, sample)
    pdb = _resolve_topology_pdb(sim_dir)
